...
if (baseUrl.toLowerCase().includes("thallo.io")) {
  throw new Error(
    `Do not run this example against the production environment as it could cause purchases to be made which are final and you will be invoiced for them at the end of the month. Baseurl: ${baseUrl}`
  );
}
...
async function run() {
  ...
  const projectIds = marketData.projects.map(
  ...
  const selectedProject = projects.filter(
    (project: { location: string }) => project.location === "SL"
  )[0];

  // get sell order for this project
  const vintage = marketData.projects.filter(
  ...
  // request retirement
  const retirementRequestId = (
    await httpClient.post("api/caas/request-retirement", {
      quantity_grams: "20",
